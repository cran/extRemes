gevparamCI.gui <- function( base.txt) {

#
# This function provides a gui for 'gev.prof' of Stuart Coles.
#
...
# 	}
	...
	nl2 <- paste( "  ", "  ", sep="\n")
if( !rl.only & !xi.only)
	msg1 <- paste( "Estmating CIs for GEV ", m.val,
			"-yr. return level and shape parameter (xi).", sep="")
else if( rl.only)
	msg1 <- paste( "Estmating CIs for GEV ", m.val, "-yr. return level.",
				sep="")
else if( xi.only) msg1 <- paste( "Estmating CIs for GEV shape parameter (xi).", 
				sep="")
if( !xi.only) {
	# msg2 <- paste(  "Return level range = ", rl.xlow.val, " to ",
	# 		rl.xup.val, sep="")
	msg2b <- paste( "Estimated return level = ", round( est.rl, digits=4),
			sep="")
...
base <- tktoplevel()
tkwm.title( base, "Estimated confidence limits for m-year GEV return level and shape parameter (xi)")
...
# Middle frame to enter arguments for 'gev.prof' fcn.

# Frame for m-year return level.
m.frm <- tkframe( mid.frm, borderwidth=2, relief="groove")
...
help.but <- tkbutton( bot.frm, text="Help", command=gevprofhelp)
...
tkbind( help.but, "<Return>", gevprofhelp)
